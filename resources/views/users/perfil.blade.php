...
.ig-username { font-size: 1.5rem; font-weight: 300; color: var(--text); letter-spacing: -.01em; }
.ig-badge { display: inline-block; padding: .25rem .85rem; border-radius: 50px; font-size: .78rem; font-weight: 700; text-transform: uppercase; letter-spacing: .04em; }
...
.btn-seguir-ig.seguindo { background: transparent; border-color: var(--border); color: var(--text); }
...
.btn-mensagem:hover { border-color: #4f46e5; color: #4f46e5; background: #eef2ff; }

.ig-stats { display: flex; gap: 2.5rem; margin-bottom: 1rem; }
...
.ig-overlay-stat { display: flex; align-items: center; gap: .4rem; color: #fff; font-size: .95rem; font-weight: 700; }
...
.ig-post-placeholder p { font-size: .72rem; color: #475569; line-height: 1.4; margin: 0; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; }

.ig-tipo-badge { position: absolute; top: 6px; right: 6px; font-size: .7rem; padding: .15rem .45rem; border-radius: 4px; font-weight: 700; backdrop-filter: blur(4px); }
...
.ig-vazio { grid-column: 1/-1; text-align: center; padding: 4rem 1rem; color: var(--muted); }
...
html.dark .ig-badge-leitor        { background: #0c4a6e; color: #7dd3fc; }
html.dark .ig-badge-editor        { background: #052e16; color: #86efac; }
html.dark .ig-badge-administrador { background: #451a03; color: #fcd34d; }
</style>
@endpush
    ...
    <div class="ig-header">
        ...
        <div class="ig-info-col">
            <div class="ig-top-row">
                ...
                @auth
                    @if(auth()->id() === $user->id)
                        {{-- Próprio perfil: engrenagem --}}
                        <a href="{{ route('profile.edit') }}" class="btn-settings" title="Configurações do perfil">⚙️</a>
                    @else
                        {{-- Outro usuário: seguir + mensagem --}}
                        ...
                        <a href="{{ route('dm.abrir', $user->id) }}" class="btn-mensagem">
                            💬 Enviar mensagem
                        </a>
                    @endif
                @endauth
    ...
    <div class="ig-grid">
        @forelse($posts as $post)
            @php
                ...
                $ytThumb = null;
                if ($post->tipo === 'video' && str_contains($post->video ?? '', 'youtu')) {
            ...
            <a href="{{ route('posts.show', $post->id) }}" class="ig-post-item">
                ...
                @elseif($post->tipo === 'texto')
                    <div class="ig-post-placeholder" style="background:linear-gradient(135deg,#dbeafe,#bfdbfe);">
                        ...
                        @if($post->texto)<p>{{ Str::limit($post->texto, 100) }}</p>@endif
                    </div>
                ...
                @else
                    <div class="ig-post-placeholder"><span class="icon">📄</span></div>
                @endif
